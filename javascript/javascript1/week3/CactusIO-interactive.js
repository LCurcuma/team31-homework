...
function showStatus(activities){
    ...
    if(activitiesLength === 1){
        console.log('You have added ' + activitiesLength + ' activity. It amount to ' + totalTime + ' min. of usage');
    } else {
    console.log('You have added ' + activitiesLength + ' activities. They amount to ' + totalTime + ' min. of usage');
    }
    }
}
...
function giveIdeaForWhatToDo(){
    const someActivities = ['learn JavaScript', 'learn HTML', 'learn CSS', 'read something', 'sleep', 'play some games', 'write poem', 'go shopping', 'go for a walk', 'watch something', 'do sports'];
...
function showTheBiggestActivity(){
    ...
    for(let i = 0; i < activities.length; i++){
        if(activities[i].duration === biggestDuration){
            console.log('You have spent the most time on ' + activities[i].activity + ': ' + biggestDuration + ' min');
        }
    }
}
